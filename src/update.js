...
export async function update(targetDir) {
  ...
  try {
    currentVersion = (
      await readFile(join(targetDir, '_nxos', '.nxos-version'), 'utf-8')
    ).trim();
  } catch {
  ...
  const newVersion = (
    await readFile(join(TEMPLATES_DIR, '_nxos', '.nxos-version'), 'utf-8')
  ).trim();
  ...
  for (const id of availableSkills) {
    if (id === 'nxos-skill-creator') continue;
