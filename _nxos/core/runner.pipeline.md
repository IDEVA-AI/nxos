...
1. You have already loaded:
   - The squad's `squad.yaml` (passed to you by the nxos skill)
   - The squad's `squad-party.csv` (all agent personas)
   - Company context from `_nxos/_memory/company.md`
...
3. **Resolve skills**: Read `squad.yaml` → `skills` section. For each non-native skill (anything other than web_search, web_fetch):
   a. Verify `skills/{skill}/SKILL.md` exists
      ...
      - If yes → read `_nxos/core/skills.engine.md`, follow Operation 2 (Install)
...
4. **Inject format context**: Check if the current step's frontmatter contains a `format:` field.
   If present:
   a. Read `_nxos/core/best-practices/{format}.md` (e.g., `_nxos/core/best-practices/instagram-feed.md`)
      - If the file does not exist → **WARNING**: "Format '{format}' not found in _nxos/core/best-practices/. Skipping format injection." Continue without format.
...
- If a step file is missing, inform the user and suggest running `/nxos edit {squad}` to fix.
