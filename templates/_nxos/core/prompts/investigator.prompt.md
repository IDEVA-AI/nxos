# Investigator — Content Investigation Template

Use this template when the Architect needs to investigate real content from reference sources during squad creation. The Investigator is NOT a squad agent — it is a tool the Architect dispatches as a subagent during Phase 1.5 (Investigation) of the create-squad workflow.

## Purpose

When a user provides reference URLs during squad discovery ("follow the style of @username" or "use this as a reference"), the Architect dispatches one Investigator subagent per source URL. Each subagent navigates to the source using Playwright MCP, extracts real content (text, captions, slide content, video transcriptions, articles, documentation, or any other extractable content), and produces a structured analysis of patterns found.
...
2. For each URL, the Architect launches an Investigator subagent in the background using the Task tool
3. Each subagent runs independently — one source per subagent
...
```
squads/{squad-name}/_investigations/{source-id}/screenshots/{filename}.png
```
...
The `{squad-name}` value is always provided by the Architect in the Investigator subagent prompt.
...
This mode applies to Instagram only. YouTube, Twitter/X, LinkedIn, and other source types use their own quantity configuration.

---

## Prerequisites Check

Before running any investigation, verify the required tools are available. Run each check as a Bash command.
...
- If Playwright MCP is unavailable, the Investigator cannot run. Inform the user and fall back to Phase 2 web research only.
- If yt-dlp, ffmpeg, or whisper are missing, the Investigator can still run but will skip video transcription. Text-based content (carousels, tweets, LinkedIn posts, text posts, articles, documentation) can still be extracted. Log a warning: "Video transcription tools not available — skipping Reel/video transcription. Text content will still be extracted."
...
The Investigator uses a persistent browser profile to preserve login sessions across investigations.

### Profile Location

The browser profile is stored at `_nxos/_browser_profile/`. This directory persists between runs so that login cookies are preserved.

### First-Time Setup

On the first investigation for a given platform, the Investigator may encounter a login wall. When this happens:
...
After the first login, cookies are preserved in the browser profile. The Investigator should still check for login walls on each run (platforms may expire sessions) and re-prompt the user if needed.

---

## Source Extractors

The Investigator can investigate ANY source accessible via a browser. It detects the source type from the URL and applies the corresponding extractor when available. For sources without a dedicated extractor, it uses a generic web extraction approach (navigate, snapshot, extract visible text and structure).

The platform-specific extractors below serve as optimized handlers for common source types. New extractors can be added for any source type.
...
The Investigator produces three output files per investigation. The first two are per-source, the third is a consolidation across all sources.

### Raw Content File: `raw-content.md`

One file per source, saved to `squads/{squad-name}/_investigations/{source-id}/raw-content.md`.
...
One file per source, saved to `squads/{squad-name}/_investigations/{source-id}/pattern-analysis.md`.
...
One file per investigation (across all sources), saved to `squads/{squad-name}/_investigations/consolidated-analysis.md`.
...
Total contents extracted: {N across all sources}

## Sources Analyzed

| # | Source | Platform | Contents | Top Content Type | Avg. Engagement |
|---|---------|----------|----------|------------------|-----------------|
...
Patterns that appear across ALL (or most) analyzed sources:

### Structural Patterns
- [Pattern found across multiple sources]: Seen in [N] of [total] sources.
  Examples: [reference specific content from different sources].
- [Pattern]: Seen in [N] of [total] sources. Examples: [references].
- [Pattern]: Seen in [N] of [total] sources. Examples: [references].

### Language Patterns
- [Shared language pattern]: Found in [N] sources. Examples: [references].
- [Shared language pattern]: Found in [N] sources. Examples: [references].
- [Shared language pattern]: Found in [N] sources. Examples: [references].

### Engagement Drivers
- [Universal engagement driver]: Correlates with high performance across
  [N] sources. Evidence: [specific examples].
- [Universal engagement driver]: Evidence: [specific examples].
- [Universal engagement driver]: Evidence: [specific examples].

## Source Differentiators

What makes each source's approach unique (not shared with others):
...
A synthesized framework based on the best patterns found across all sources.
...
patterns across all analyzed sources:

1. **Opening/Hook**: [Recommended approach based on top-performing hooks across
   all sources. Include 3 hook patterns that worked best, with templates.]
...
4. **Closing/CTA**: [Recommended CTA approach synthesized from highest-performing
   CTAs across all sources. Include 2-3 CTA templates.]

### Voice Guidelines
Synthesized voice recommendations based on patterns across all sources:
...
## How the Architect Uses Investigator Output
...
- **Reviewer agent persona**: Quality Criteria are calibrated against real engagement metrics from investigated sources. The reviewer knows what "high-performing" looks like because it has seen real examples.
...
2. Patterns found across multiple investigated sources carry more weight than single-source patterns
...
The Investigator detects the source type from the URL to apply the correct extractor:
...
| Any other URL | Generic Web | Generic Web Extractor (navigate, snapshot, extract text) |

For unrecognized URLs, the Investigator uses a generic extraction approach: navigate to the page, take snapshots, extract all visible text and structural elements. This allows investigating blogs, documentation sites, forums, newsletters, or any other web-accessible source.

### Configuration Prompts

Before starting extraction for each source, the Investigator asks the user for configuration using numbered lists (one question at a time):
...
The Investigator recommends extraction settings based on the squad type:
...
- Maximum time per source: 10 minutes. If extraction exceeds this, save what has been collected so far and note: "Investigation truncated at {N} contents due to time limit."
- If a platform blocks access or returns errors, retry once. If the retry fails, inform the user and skip that source.
