---
name: nxos-dev
description: "nxos development checklist — verifies templates sync, package integrity, and distribution correctness."
---

# nxos Development Checklist

You are running the nxos-dev verification skill inside the nxos repository.
Your job is to detect and report distribution issues before they reach users.

## How nxos Distribution Works

Understand this before checking anything:

- **`templates/`** → Copied by `src/init.js:copyCommonTemplates()` during `npx nxos init`
  and by `src/update.js` during `npx nxos update`. Everything in `templates/` except
...
- **`agents/`** (project root) → Predefined agent catalog, distributed via npm
  (`package.json files[]`). Auto-installed during `npx nxos init` and new agents
  added during `npx nxos update`. Protected from overwrites in
  `src/update.js:PROTECTED_PATHS`. Users can also install manually via
  `npx nxos agents install`.
...
- **`skills/`** (project root) → Bundled skills catalog, distributed via npm
  ...
  during init. Users can also install manually via `npx nxos skills install`.

- **`_nxos/core/*`** → Framework core files. MUST have a mirror copy in
  `templates/_nxos/core/*`. Any change to a core file that isn't synced to templates
  means `npx nxos init` delivers STALE content to new users.

- **`skills/*`** → Bundled skills catalog. Two distribution sub-types:
  - **Multi-file skills** (have scripts/assets/agents dirs alongside SKILL.md, e.g. `nxos-skill-creator`):
    also have a mirror in `templates/skills/*` so the full directory structure is copied during init.
    Check B verifies this sync.
  - **Single-file skills** (only `SKILL.md`, no subdirs, e.g. `nxos-agent-creator`):
...
- **`src/update.js:PROTECTED_PATHS`** → Directories NEVER overwritten during update:
  `_nxos/_memory`, `_nxos/_investigations`, `agents`, `squads`.

## Multi-IDE Architecture

nxos supports multiple IDEs. When a user runs `npx nxos init`, they choose which IDE(s) to install for. Files are copied from two places:
...
|------|-----------|----------|
| SHARED | Applies to all IDEs equally | `_nxos/core/`, `templates/` (excluding `ide-templates/`) |
...
| Antigravity | `templates/ide-templates/antigravity/` | `.antigravity/rules.md`, `.agent/workflows/nxos.md` |
...
- ❌ Wrong: Edit `_nxos/core/runner.pipeline.md` adding `if antigravity: run sequentially`
...
- ✅ Fine: Edit `_nxos/core/architect.agent.yaml` because the change benefits ALL IDEs equally.
...
#### Check A: Core file sync (`_nxos/core/**` changed)

For EACH changed file matching `_nxos/core/**`:

1. Compute the expected template path: `templates/{same relative path}`
   Example: `_nxos/core/runner.pipeline.md` → `templates/_nxos/core/runner.pipeline.md`

2. Run diff:
   ```bash
   diff "_nxos/core/{file}" "templates/_nxos/core/{file}"
   ```

3. If diff shows differences:
   - **FAIL**: Report the file and show the diff summary
   - **Fix**: `cp _nxos/core/{file} templates/_nxos/core/{file}`

4. If template file doesn't exist:
   - **FAIL**: "Template missing for `_nxos/core/{file}`"
   - **Fix**: `mkdir -p templates/_nxos/core/{dir} && cp _nxos/core/{file} templates/_nxos/core/{file}`
...
3. Verify it includes all user-owned directories:
   - `_nxos/_memory` (user preferences and company context)
   - `_nxos/_investigations` (Investigator investigation data)
...
5. Verify `installNonMcpSkills` filters out: `nxos-skill-creator`, type `mcp`, type `hybrid`, skills with `env`
...
#### Check J: IDE contamination in shared files (any `_nxos/core/**` or `templates/**` changed, excluding `templates/ide-templates/**`)

Scan shared files for IDE-specific conditional logic that should live in `templates/ide-templates/{ide}/` instead.

**Files to scan:**
- All files in `_nxos/core/`
...
**Exclusions:** Mentions inside this nxos-dev SKILL.md itself (documentation), and any comment explicitly labeled as a cross-reference.
